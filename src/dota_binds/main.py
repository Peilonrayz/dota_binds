...
class bind(object):
    ...
    def __init__(self, command, bKey, key, up=False):
        ...
        self.string = 'alias "'+self.name+'" "bind '+key.lower()+' '+command+'"'
        if up:
            bKey.append(self)
        else:
...
class bKey(object):
    ...
    def getBinds(self):
        ...
        if self.base != None:
            ...
            string += 'alias "+mod'+self.upper()+'" "'+';'.join(i.name for i in self.down)+'"\n'
...
class bindHolder(object):
    ...
    def saveForm(self):
        return [self.comm.saveForm()]+[self.base.saveForm()]+[[i.saveForm() for i in self.lst]]

b = bindHolder()
m4 = bKey('mouse4', b)
b.comm.add(command('sFollow', ''))
bind(command('top', 'dota_camera_setpos -2296.339355 1085.593506 0.000000', 'sFollow'), m4, '1')
bind(command('bot', 'dota_camera_setpos 2874.552734 -3017.180664 0.000000', 'sFollow'), m4, '1', True)
bind(command('tShop', 'toggleshoppanel'), m4, '2')
bind(command('sToggle', 'dota_smart_camera_toggle'), m4, '2', True)
bind(command('home', 'dota_select_courier;dota_ability_execute 0;+camera;dota_courier_burst'), m4, '3')
bind(command('secret', 'dota_select_courier;dota_ability_execute 1;+camera;dota_courier_burst'), m4, '3', True)
