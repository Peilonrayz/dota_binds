class alias(object):
    def _init(self, string, name, bind):
        if name==None:name = get(string, '(', ')')
        if bind==None:bind = get(string, '`', '`')
        down = get(string, '\'', '\'')
        up = get(string, '"', '"')
    ...
    def add(self, string, name, bind, b, n):
        ...
        if down != None:self.down.append(down)
        if up != None:self.up.append(up)
    ...
    def __str__(self):
        ...
        if self.up or len(self.down) > 1:
            return 'alias "+{0}" "{1}"\n{4}alias "{2}" "{3}+{0}"'.format(
                ...
                ';'.join(self.down),
                self.name,
                '' if self.bind == None or len(self.bind) == 0 else 'bind {} '.format(self.bind),
                'alias "-{0}" "{1}"\n'.format(self.name.upper(), ';'.join(self.up)) if self.up else ''
            )
        else:
            return 'alias "{}" "{}{}"'.format(
                self.name,
...
class modifyer(object):
    def __init__(self, string, bind=None, name=None):
        if name==None:self.name = get(string, '(', ')')
        else:self.name = name
        if bind==None:self.bind = get(string, '|', '|')
        else:self.bind = bind
        self.d = True
        self.mod = get(string, '[', ']')
        if self.mod == None:
    ...
    def _mid(self):
        return '{}\n{}\n'.format(
            'alias "+{}" "{}"'.format(self.name.upper(),';'.join(i.name for i in (self.down+self.mod.up+self.pred))),
            'alias "-{}" "{}"'.format(self.name.upper(),';'.join(i.name for i in (self.up+self.mod.down+self.preu)))
        )
    def _lat(self):
        ...
        if down:
            ...
            if self.mod.name == '':
                ...
                return 'alias "{}" "bind {} +{}"'.format(self.name, self.bind, self.name.upper())
    def __str__(self):
        if self.name == '':
            return self._form()
        else:
    ...
    def call(self):
        if len(self.name) > 0:
            return '+{0}\n-{0}'.format(self.name.capitalize()+'_Bonus' if (self.sDown+self.mod.sUp) else self.name.upper())
        else:
            return ''
    
def get(string, l1, l2):
    string = string.replace('\\\\', '__\\\\__').replace('\\'+l1, '').replace('\\'+l2, '').replace('__\\\\__', '\\')
...
def openFile(fileName):
    ...
    print plane
    place = '/home/peilonrayz/.local/share/Steam/SteamApps/common/dota 2 beta/dota/cfg/'+fileName[:-4]+'.cfg'
    print place
